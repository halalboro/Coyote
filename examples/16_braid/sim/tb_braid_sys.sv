 ...
 *
 * tb_braid.sv proves the protocol. THIS testbench predicts the full one-way
 ...
 * stage count here comes from OUR MEASUREMENT -- 55.6 ns via `braid bench -l 2`
...
module tb_braid_path #(
    ...
    // 10.3125 Gbps, from `braid bench -l 2`) plus ~9.6 ns of cable. Not AMD's
...
module tb_braid_sys;
    ...
    tb_braid_path #(.N_STAB(N_STAB), .N_CORR(N_CORR), .GT_STAGES(STAGES_NOW)) u_now (
    ...
    tb_braid_path #(.N_STAB(N_STAB), .N_CORR(N_CORR), .GT_STAGES(STAGES_NEXT)) u_next (
    ...
    initial begin
        ...
        $display("  (braid bench -l 2), plus ~9.6 ns of cable.\n");
