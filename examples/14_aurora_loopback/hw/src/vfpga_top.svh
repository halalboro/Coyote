...
// Read-data mux. `csr` is pure host-written storage with a single driver
// (the always_ff above). The read-only status registers (STATUS, RX_BEAT_CNT,
// RX_MISMATCHES) are overlaid here combinationally so they never drive `csr`
// directly — driving `csr` from a second process trips Vivado DRC MDRV-1.
logic [ADDR_MSB-1:0] rd_idx;
assign rd_idx = ar_addr_q[REG_ADDR_BITS-1:ADDR_LSB];

always_comb begin
    case (rd_idx)
        // bit0=tx_done, bit1=rx_done, bit2=channel_up, bits[6:3]=lane_up
        3'd1:    axi_ctrl.rdata = {57'b0, aurora_lane_up, aurora_channel_up, rx_done, tx_done};
        3'd3:    axi_ctrl.rdata = rx_beat_cnt;
        3'd4:    axi_ctrl.rdata = rx_mismatches;
        default: axi_ctrl.rdata = csr[rd_idx];
    endcase
end
...
// Note: STATUS / RX_BEAT_CNT / RX_MISMATCHES are exposed to the host through
// the read-data mux above (axi_ctrl.rdata), NOT by writing into `csr`. This
// keeps `csr` single-driver and avoids Vivado DRC MDRV-1.
