/**
 * QLINK vFPGA-side PHY shim
 *
 * Adapts qlink_link's 32-bit word ports to the 256-bit AXI4-Stream that the
 ...
 * was a second register behind qlink_link_tx's own output register, worth 3.9 ns
 * of pure latency. qlink_link_tx already drives phy_tx_* from flops, so the path
 ...
 * not-in-table), packed there by qlink_gty_wrapper. qlink_link_rx folds it into
 * frame validity alongside its own checksum.
 */

module qlink_phy_shim (
