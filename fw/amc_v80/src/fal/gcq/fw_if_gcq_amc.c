...
static uint32_t prvGCQOpen( void *pvFWIf )
{
    ...
    {
        /* Coyote AMC bringup: use in-memory (DDR ring-header) produced pointers
         * instead of the GCQ IP tail-pointer registers. On the Coyote V80 shell
         * the host (PCIe) can READ gcq_m2r/S00 registers but its WRITES to them
         * are dropped by the interconnect, so a register-based submit doorbell
         * never reaches the R5. In-mem-ptr mode keeps the SQ/CQ produced pointers
         * in DDR (ring+36/+40), which the host CAN write and the R5 reads over the
         * NoC — the whole mailbox becomes pure DDR and bypasses the GCQ registers.
         * Polling mode means the (register-triggered) irq_sq is unused anyway.
         * Original AVED value: xFlags = 0.
         * The host-side client must set the matching flag. */
        GCQ_FLAGS_TYPE xFlags = GCQ_FLAGS_TYPE_IN_MEM_PTR_ENABLE;
