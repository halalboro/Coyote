...
/* Coyote AMC bringup: no SMBus PL IP in Coyote's V80 shell (AVED instantiates
 * axi_smbus_rpu at 0x80044000, Coyote does not). The AMC ping firmware doesn't
 * exercise the SMBus code path at runtime, but the address is referenced from
 * profile_fal.c at compile time. Stub to 0 so the build proceeds; any actual
 * SMBus access at runtime would fault (a Phase B item once SMBus IP is added).
 *
 * Original AVED value, restore for AVED reference HW:
 *     HAL_SMBUS_BASE_ADDR = XPAR_SMBUS_0_BASEADDR
 */
...
/* Coyote AMC bringup: shared-memory region relocated from AVED-canonical
 * 0x38000000 (896 MB into DDR4) to 0x00100000 (1 MB into DDR4) so it falls
 * inside a PCIe-visible 32 MB BAR4 window. AMC's .text/.data still live at
 * 0x40000000 per lscript.ld and don't overlap with this 4 MB region.
 *
 * Original AVED values, restore for AVED reference HW:
 *     HAL_RPU_SHARED_MEMORY_BASE_ADDR = 0x38000000
 *     HAL_RPU_SHARED_MEMORY_END_ADDR  = 0x3FFFF000
 */
#define HAL_RPU_SHARED_MEMORY_BASE_ADDR         ( 0x00100000 )
#define HAL_RPU_SHARED_MEMORY_END_ADDR          ( 0x004FF000 )
...
/* Coyote AMC bringup: GCQ is instantiated at BD top level (`gcq_m2r/S01_AXI`),
 * not nested inside a `base_logic/` hierarchy as in AVED. xparameters.h picks
 * up the shorter name accordingly.
 *
 * Original AVED value, restore for AVED reference HW:
 *     HAL_BASE_LOGIC_GCQ_M2R_S01_AXI_BASEADDR = XPAR_BASE_LOGIC_GCQ_M2R_BASEADDR
 */
#define HAL_BASE_LOGIC_GCQ_M2R_S01_AXI_BASEADDR ( XPAR_GCQ_M2R_BASEADDR )
