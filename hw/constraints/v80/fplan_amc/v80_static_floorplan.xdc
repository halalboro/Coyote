# =============================================================================
# V80 static floorplan -- AMC bringup (Coyote example 15). Coyote AMC bringup.
# =============================================================================
# WHY THIS FILE EXISTS (do not merge into ../fplan/v80_static_floorplan.xdc):
#
#   The shared static floorplan (../fplan/v80_static_floorplan.xdc) was
#   restructured for the V80 vFPGA mesh ("Strategy B", commit ad253e66 "Bigger
#   mesh for V80 + inter vFPGA NoC routing", + later WIP re-enlargement). That
#   restructuring shrinks pblock_inst_shell to SLR0 and re-carries only the
#   X1-column NoC tiles (NOC_NSU512_X1Y4:X1Y6) into the shell pblock. It leaves
#   the X0-column axi_noc_0 M00 slave unit (NOC_NSU512_X0Y4, clock region X1Y3)
#   without a routable GND tie-off for its hardened sideband pins
#   (NSU_USR_INTERRUPT_IN_0..3, IF_NOC_AXI_RUSER_9), so route_design fails with
#   "[Route 35-7] ... unroutable pins ... GNDNet not completely routed".
#
#   Example 15 is an N=1 AMC build (EN_PR=0, no external vFPGA floorplan) and
#   needs none of the mesh. It uses this pre-mesh "Strategy A" floorplan --
#   verbatim from ad253e66~1 -- which shell-owns the SLR1/SLR2 corridors and
#   routed cleanly for the working Jul-20 PDI. link.tcl selects this directory
#   instead of ../fplan when cfg(en_amc)==1 on V80.
#
#   If AMC is ever merged with the vFPGA mesh (EN_PR=1), this isolation must be
#   revisited: the shell can no longer own SLR1/SLR2 while vFPGAs live there.
# =============================================================================
