###
### BRAID GTY on QSFP1 — single lane, 10.3125 Gbps, 8B/10B
###
### Sorts alphabetically AFTER u280_shell_net_1.xdc (hence the `zbraid` prefix)
### so its PACKAGE_PIN assignments win. Vivado warns on the override; the last
### assignment is the one that takes effect.
###
### Active when EN_BRAID_GTY=1. Mutually exclusive with EN_NET_1 and
...
### assigns to gt1_*[0], and the LOC filter matches *inst_braid* with -quiet,
### so it is a no-op when no BRAID cell exists. The reverse holds for
### u280_shell_zaurora_1.xdc during a BRAID build. Keep it that way: if you add
### assignments here that differ from Aurora's, they WILL leak into Aurora
### builds, because "zbraid" sorts after "zaurora" and the last write wins.
###
...
set_property LOC GTYE4_CHANNEL_X0Y44 [get_cells -hierarchical -quiet \
    -filter {NAME =~ *inst_braid*GTYE4_CHANNEL_PRIM_INST}]
