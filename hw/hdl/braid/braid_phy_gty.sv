 ...
 * braid_framer_raw, which is instantiated here and is the SAME module the
 * testbench exercises (examples/16_braid/sim/tb_braid.sv). Keep it that way:
 ...
 * braid_framer_raw instead -- see that file for the wire protocol.
 * TXPROGDIV_FREQ_VAL lands on the SAME 390.625 MHz fabric clock the 8B/10B
 * configuration used (see scripts/ip_inst/braid_infrastructure.tcl), so this
...
module braid_phy_gty (
    ...
    // lives in its own domain, which is why braid_framer_raw takes two clocks.
    ...
    // OURS: it lives in braid_framer_raw's rxslide-hunt FSM, and the only thing
    ...
    // gtwiz_reset_all. `braid bench -l 2` -- the FIRST command in the bring-up
