/**
 * BRAID framer — RAW mode
 *
 * Replaces braid_framer. The GT no longer does 8B/10B or gearboxing, which is
...
module braid_framer_raw #(
    ...
    // ================================================================ TX
    // FIX (round 3): no registered T_HDR stage. braid_framer (8B/10B) emits
    ...
    braid_scrambler #(.INVERT(0)) u_scr (
    ...
    // the same reason: braid_scrambler's transform is scr_in XORed with a
    ...
    // (mirroring braid_framer's w_marker mux), not one cycle later.
    always_comb begin
        case (tx_state)
    ...
    // UNREACHABLE by construction: braid_link_tx holds phy_tx_valid high for a
    ...
    always_ff @(posedge clk_tx)
        if (rstn_tx && tx_state == T_DATA && !phy_tx_valid)
            $error("braid_framer_raw: mid-frame bubble on phy_tx_valid. The RX has no matching skip, so the scramblers will diverge permanently. See the scrambler-state invariant comment.");
    ...
    braid_scrambler #(.INVERT(1)) u_des (
    ...
    // safe here: braid_link_rx (`if (phy_rx_sof) ... else if (phy_rx_eof)
    ...
    // deliberately trying it: tb_braid's back-to-back-frames test broke.
    //
    // The actual fix is a closed-form, zero-extra-hop equivalent for des_out
    // itself, derived from braid_scrambler's own structure. Inside its loop,
    ...
    // braid_scrambler's output is exactly d_in[i] ^ (state_in[57-i] ^
    ...
    // function braid_scrambler computes, for every state, every word, every
    ...
    // braid_framer (8B/10B) drives phy_rx_data/hdr/cks/type/sof/eof/valid
    ...
    // must stay exactly this fast regardless (braid_link_rx's
    ...
    // inventing a signal; the frame checksum in braid_link_rx is the
        ...
        if (!rstn_rx) begin
            ...
            case (al_state)
                ...
                A_LOCKED: begin
                    ...
                    // tb_braid_align proves no non-zero rotation of 5C3D_2A96
                    ...
                    // (braid_link_tx never emits an empty frame, and the R_IDLE
    ...
    // What trips now: a sustained run of words with NOT ONE ALIGN among
                    ...
                    // tb_braid_raw.sv caught that; it is the reason this
    ...
    always_ff @(posedge clk_rx) begin
        if (!rstn_rx) begin
            ...
            //
            // What makes that safe rather than reckless: braid_link_tx holds
